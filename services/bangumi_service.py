...
- BgmList 提供精确播出时间（中文网站，数据准确）。
- 两者结合：Bangumi 负责内容，BgmList 负责时间。
...
from services.bgmlist_service import BgmListService
...
class BangumiService:
    ...
    def __init__(self, config: Config) -> None:
        ...
        self._bgmlist = BgmListService(config)  # 用于获取播出时间
    ...
    async def search_anime(self, title: str) -> dict | None:
        ...
        try:
            ...
            # 使用 BgmList 获取实际播出时间（用中文标题）
            title_cn = anime.get("name_cn", anime.get("name", ""))
            if title_cn:
                airing_time = await self._bgmlist.get_anime_airing_time(title_cn)
                if airing_time:
    ...
    async def get_current_season_by_day(self) -> dict[int, list[dict]]:
        """获取当前季度新番，按星期几分组（Bangumi 内容 + BgmList 时间）。
        ...
        """
        try:
            # 1. 先获取 BgmList 的整周播出时间表（中文网站，数据准确）
            bgmlist_schedule = await self._bgmlist.get_weekly_schedule()

            # 2. 获取 Bangumi 的每日放送（中文内容）
            bangumi_result = await self._request("/calendar")

            if not bangumi_result:
                logger.warning("Bangumi 季度查询无结果，使用纯 BgmList 数据")
                return bgmlist_schedule

            # 3. 合并：Bangumi 内容 + BgmList 时间
            animes_by_day = {}
            for day_data in bangumi_result:
                ...
                if 0 <= weekday <= 6:
                    ...
                    for anime in sorted_items:
                        ...
                        # 尝试从 BgmList 匹配播出时间（用中文标题匹配）
                        title_cn = anime.get("name_cn", anime.get("name", "")).strip()
                        if title_cn and weekday in bgmlist_schedule:
                            matched_time = self._fuzzy_match_bgmlist_time(
                                title_cn, bgmlist_schedule[weekday]
                            )
                            if matched_time:
                                normalized["air_time"] = matched_time
                            else:
                                # 如果 BgmList 没有匹配到，显示「未知时间」而不是 Bangumi 的日期
                                normalized["air_time"] = "未知时间"

                        animes_by_day[weekday].append(normalized)

            logger.info("Bangumi + BgmList 季度查询成功: %d 天有新番", len(animes_by_day))
            return animes_by_day

        except Exception as exc:
            logger.error("Bangumi 季度查询失败: %s", exc)
            # 失败时回退到纯 BgmList 数据
            return await self._bgmlist.get_weekly_schedule()

    def _fuzzy_match_bgmlist_time(
        self, title_cn: str, bgmlist_animes: list[dict]
    ) -> str | None:
        """模糊匹配 BgmList 播出时间。
        ...
        Args:
            ...
            bgmlist_animes: BgmList 同一天的番剧列表。
        ...
        for bgmlist_anime in bgmlist_animes:
            bgmlist_title = bgmlist_anime.get("title", {})
            bgmlist_native = bgmlist_title.get("native", "").strip()

            # 策略 1: 完全匹配（忽略大小写和空格）
            if normalize(bgmlist_native) == title_norm:
                return bgmlist_anime.get("air_time")

            # 策略 2: 包含匹配
            if (title_norm in normalize(bgmlist_native) or
                normalize(bgmlist_native) in title_norm):
                return bgmlist_anime.get("air_time")
    ...
    def _normalize_anime(self, anime: dict) -> dict:
        ...
        return {
            ...
            "air_time": air_date,  # 默认使用 air_date，后续可能被 BgmList 覆盖
        }
