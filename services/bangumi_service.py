...
- 弹弹play 提供精确播出时间（中文网站，数据准确）。
- 两者结合：Bangumi 负责内容，弹弹play 负责时间。
...
from services.dandanplay_service import DandanplayService
...
class BangumiService:
    ...
    def __init__(self, config: Config) -> None:
        ...
        self._dandanplay = DandanplayService(config)  # 用于获取播出时间
    ...
    async def search_anime(self, title: str) -> dict | None:
        ...
        try:
            ...
            # 使用弹弹play 获取实际播出时间（用中文标题）
            title_cn = anime.get("name_cn", anime.get("name", ""))
            if title_cn:
                airing_time = await self._dandanplay.get_anime_airing_time(title_cn)
                if airing_time:
    ...
    async def get_current_season_by_day(self) -> dict[int, list[dict]]:
        """获取当前季度新番，按星期几分组（Bangumi 内容 + 弹弹play 时间）。
        ...
        """
        try:
            # 1. 先获取弹弹play 的整周播出时间表（中文网站，数据准确）
            dandanplay_schedule = await self._dandanplay.get_weekly_schedule()

            # 2. 获取 Bangumi 的每日放送（中文内容）
            bangumi_result = await self._request("/calendar")

            if not bangumi_result:
                logger.warning("Bangumi 季度查询无结果，使用纯弹弹play 数据")
                return dandanplay_schedule

            # 3. 合并：Bangumi 内容 + 弹弹play 时间
            animes_by_day = {}
            for day_data in bangumi_result:
                ...
                if 0 <= weekday <= 6:
                    ...
                    for anime in sorted_items:
                        ...
                        # 尝试从弹弹play 匹配播出时间（用中文标题匹配）
                        title_cn = anime.get("name_cn", anime.get("name", "")).strip()
                        if title_cn and weekday in dandanplay_schedule:
                            matched_time = self._fuzzy_match_dandanplay_time(
                                title_cn, dandanplay_schedule[weekday]
                            )
                            if matched_time:
                                normalized["air_time"] = matched_time
                            else:
                                # 如果弹弹play 没有匹配到，显示「未知时间」而不是 Bangumi 的日期
                                normalized["air_time"] = "未知时间"

                        animes_by_day[weekday].append(normalized)

            logger.info("Bangumi + 弹弹play 季度查询成功: %d 天有新番", len(animes_by_day))
            return animes_by_day

        except Exception as exc:
            logger.error("Bangumi 季度查询失败: %s", exc)
            # 失败时回退到纯弹弹play 数据
            return await self._dandanplay.get_weekly_schedule()

    def _fuzzy_match_dandanplay_time(
        self, title_cn: str, dandanplay_animes: list[dict]
    ) -> str | None:
        """模糊匹配弹弹play 播出时间。
        ...
        Args:
            ...
            dandanplay_animes: 弹弹play 同一天的番剧列表。
        ...
        for dandanplay_anime in dandanplay_animes:
            dandanplay_title = dandanplay_anime.get("title", {})
            dandanplay_native = dandanplay_title.get("native", "").strip()

            # 策略 1: 完全匹配（忽略大小写和空格）
            if normalize(dandanplay_native) == title_norm:
                return dandanplay_anime.get("air_time")

            # 策略 2: 包含匹配
            if (title_norm in normalize(dandanplay_native) or
                normalize(dandanplay_native) in title_norm):
                return dandanplay_anime.get("air_time")
    ...
    def _normalize_anime(self, anime: dict) -> dict:
        ...
        return {
            ...
            "air_time": air_date,  # 默认使用 air_date，后续可能被弹弹play 覆盖
        }
